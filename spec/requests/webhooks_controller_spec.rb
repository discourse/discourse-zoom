...
describe Zoom::WebhooksController do
  ...
  describe "authorized webhooks" do
    ...
    fab!(:webinar) do
      Fabricate(:webinar, topic: topic, zoom_id: 123, duration: 60)
    end
    ...
    before do
      ...
      described_class
        .any_instance
        .stubs(:ensure_webhook_authenticity)
        .returns(true)
    end
    ...
    describe "#webinar_started" do
      it "updates webinar status when it starts" do
        post "/zoom/webhooks/webinars.json",
             params: {
               webhook: webinar_started(zoom_id: 123)
             }
        expect(response.status).to eq(200)
        webinar.reload
    ...
    describe "#webinar_updated" do
      it "updates starts_at and ends_at when start_time changes" do
        ...
        expect(webinar.ends_at).to be_within_one_second_of(
      ...
      it "updates starts_at and ends_at when start_time and duration change" do
        ...
        expect(webinar.duration).to eq(duration)
        expect(webinar.ends_at).to be_within_one_second_of(
          webinar.starts_at + duration.minutes
        )
      end
      ...
      it "updates settings" do
        ...
        expect(response.status).to eq(200)
        webinar.reload
        expect(webinar.approval_type).to eq(
          Webinar.approval_types.key(approval_type)
        )
    ...
    describe "#webinar_participant_left" do
      it "fires a DiscourseEvent" do
        DiscourseEvent
          .expects(:trigger)
          .with() { |value| value === :webinar_participant_left }
          .once
        post "/zoom/webhooks/webinars.json",
             params: {
