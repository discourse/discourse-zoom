...
RSpec.describe Zoom::SendWebinarReminders do
  ...
  let(:needs_reminding) { Fabricate(:webinar, topic: topic_1, starts_at: DateTime.now + 20.minutes) }
  ...
  describe "with zoom_send_reminder_minutes_before_webinar set to 0" do
    before do
      SiteSetting.zoom_send_reminder_minutes_before_webinar = ''
    end

    it "does not send reminders" do
      SystemMessage.any_instance.expects(:create).with() { |value| value == "webinar_reminder" }.never
  ...
  describe "with zoom_send_reminder_minutes_before_webinar set to 0" do
    before do
      SiteSetting.zoom_send_reminder_minutes_before_webinar = "0"
    end
  ...
  describe "with zoom_send_reminder_minutes_before_webinar set to 30 minutes" do
    before do
      SiteSetting.zoom_send_reminder_minutes_before_webinar = "30"
    end
    it 'sends reminders for upcoming webinars' do
      expect(needs_reminding.reload.reminders_sent_at).to eq(nil)
      SystemMessage.any_instance.expects(:create).with() { |value| value == "webinar_reminder" }.twice
